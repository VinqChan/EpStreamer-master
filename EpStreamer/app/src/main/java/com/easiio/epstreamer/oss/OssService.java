package com.easiio.epstreamer.oss;
...
//        OSSClient的生命周期和应用程序的生命周期保持一致即可。在应用程序启动时创建一个ossClient，在应用程序结束时销毁即可。
        ...
        OSSLog.enableLog();
        if (mOssService == null) {
            mOssService = new OssService(oss, Config.BUCKET_NAME, uiDisplayer);
        }
        return mOssService;

    }

    public void asyncPutFile(final String object, final String localFile, final IuploadFileCall callback) {
        final long upload_start = System.currentTimeMillis();
        OSSLog.logDebug("upload start=" + object);
        ...
        metadata.setContentDisposition("attachment=" + file.getName());
        ...
        OSSAsyncTask task = mOss.asyncPutObject(put, new OSSCompletedCallback<PutObjectRequest, PutObjectResult>() {
            ...
            public void onFailure(PutObjectRequest request, ClientException clientExcepion, ServiceException serviceException) {
                ...
                callback.faile(localFile);
                asyncPutFile(object, localFile, callback);
...
//                mDisplayer.displayInfo(result.getServerCallbackReturnBody());
    ...
    public interface IuploadFileCall {
        void success(String url);

        void faile(String url);
    }
}
